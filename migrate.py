...
def generateJs(module,content):
    ...
    add_arr = ''
    for i in content:
        add_arr = add_arr + "'%s': $('#%s').val()," %(i[0],i[0])
    postAdd_url = '/%s/%s/postAdd' % (c , a)
    edit_url = '/%s/%s/edit' % (c , a)
    edit_title = 'edit %s' %a
    edit_arr = add_arr + "'id': $('#id').val(),"
    postEdit_url = '/%s/%s/postEdit' % (c , a)
    del_url = '%s/%s/del' % (c ,a)

    return strand % (name,add_url,add_title,add_arr,postAdd_url,edit_url,edit_title,edit_arr,postEdit_url,del_url)
...
class menuHelper
{
        public static string[string][] getData(string index = "0")
        {
                auto data = adminMenuModel.getInstance.getDataByWhere(["parent_id":index],100,0);
                return data;	
        }
...
if __name__ == "__main__":
    ...
    print(generateJs(module,content))
